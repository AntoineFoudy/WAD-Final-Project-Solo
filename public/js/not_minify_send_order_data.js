...
function send_data() {
    ...
    let user = {
        name: document.getElementById("get_name").value.trim(),
        email: document.getElementById("get_email").value.trim()
    };
    ...
    if(user && veg && amount && county && delivery) {
        let send_data = {buyer, user, veg, amount, county, delivery}
...
function send_data_to_app(send_data) {
    ...
    // Used code found on StackOver Follow at https://stackoverflow.com/questions/39735496/redirect-after-a-fetch-post-call
    .then(response => {
